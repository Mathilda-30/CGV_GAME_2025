...
function loadPalmTreeModel() {
  ...
  return new Promise((resolve, reject) => {
    gltfLoader.load(
      ...
      (progress) => {
        console.log(`Loading palm tree: ${(progress.loaded / progress.total * 100).toFixed(1)}%`);
      },
      (error) => {
...
function createBeachDetails(scene) {
  ...
  // Sea shells and pebbles scattered on beach
  for (let i = 0; i < 50; i++) {
    ...
    const shell = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ 
        color: Math.random() > 0.5 ? 0xFFEBCD : 0xFFD700, // Shell colors
        roughness: 0.8
      })
    );
...
function createAmbientLife(scene) {
  ...
  for (let i = 0; i < lifeCount * 3; i += 3) {
    ...
    positions[i + 1] = 3 + Math.random() * 8; // Height above ground
...
export function animateIslandElements() {
  if (ambientLife) {
    animateAmbientLife();
  }
}
...
function plantAllTrees(scene, world, RAPIER) {
  ...
  const testPositions = [
    [5, 5], [10, 10], [15, 15], [-10, 10], 
    [10, -10], [-15, -15], [20, 0], [0, 20],
    [25, 5], [-5, 25], [30, -15], [-20, 20],
    [35, 10], [-35, 10], [10, 35], [-10, 35]
  ];

  for (let [x, z] of testPositions) {
    if (treesPlanted >= treeCount) break;
    if (createTree(scene, world, RAPIER, x, z)) {
      treesPlanted++;
    }
  }
...
export function createIsland(scene, world, RAPIER) {
  ...
  if (groundCollider) {
    console.log(`[DEBUG] Island trimesh physics created.`);
  }
  ...
  const water = new Water(
      waterGeo,
      {
          textureWidth: 512,
          textureHeight: 512,
          waterNormals: waterNormals, // <-- Use your texture
          sunDirection: new THREE.Vector3(50, 50, 50).normalize(), // Match sun
          sunColor: 0xffffff,
          waterColor: 0x0099ff, // Lighter, clear tropical blue
          distortionScale: 2.0,  // Calmer waves (lower is calmer)
          fog: scene.fog !== undefined,
          alpha: 0.8 // <-- This makes it 90% transparent
      }
  );

  water.rotation.x = -Math.PI / 2;
  water.position.y = -0.5; // Place it slightly below the terrain's base
  scene.add(water);
