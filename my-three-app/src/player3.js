// player3.js - FULLY FIXED & WORKING
  ...
  loader.load(
    ...
    function (gltf) {
      ...
      if (gltf.animations && gltf.animations.length) {
        ...
        console.log('🎮 Available animations:', Object.keys(player.actions));
      }
      ...
      console.log('✅ Player initialized at', startPos, '(will fall to ground)');
    },
    (xhr) => { 
      console.log('📦 Player model loading:', (xhr.loaded / xhr.total * 100).toFixed(1) + '%');
    },
    (error) => { 
...
export function updatePlayer(deltaTime) {
  ...
  frameCount++;
  const shouldDebugLog = frameCount % 60 === 0;
  ...
  if (shouldDebugLog) {
    ...
    console.log(`📊 Grounded: ${player.isGrounded}, Pos: (${pos.x.toFixed(1)}, ${pos.y.toFixed(1)}, ${pos.z.toFixed(1)})`);
  }
  ...
  if (shouldDebugLog) {
    console.log(`🎮 Moving: ${isMovingHorizontally}, Grounded: ${player.isGrounded}`);
  }
...
function updateAnimationState(isMovingHorizontally) {
  ...
  if (newAction !== player.debugLastAction) {
    console.log(`📹 Animation: ${player.debugLastAction} → ${newAction}`);
    player.debugLastAction = newAction;
  }
